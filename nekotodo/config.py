...
3. 只为"已有任务数为 0 的条目"生成任务(清单中每条都标注了已有任务数),用 create_task 创建:
   - 一个条目可拆出多个任务,确保每一个任务都是最小不可再拆。
   - 用 source_item_id 把任务关联到对应条目。
   - 分类优先复用概览里已有的分类,避免制造近义的新分类;仅当内容明显不属于任何已有分类时才新建。
   - 按内容中的约束设置 deadline 与 category;任务的补充说明、要求写进 details(任务细节，无需包括截止日期等已在其余字段表达的内容)。
...
6. 全部完成后,输出一条不含工具调用的"结束",表示结束。

注意！！！！生成的内容不要包含本工作流程、要求等信息。
